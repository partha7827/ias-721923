...
disp(sprintf('noise standard deviation: %g', sigma));
if use_mex
...
for f = 1:max_frames
    ...
    [noisy_images(:,:,f) noise_data] = add_noise(image, char(noise(index)), sigma); %#ok<AGROW>
    ...
    for i = 1:length(type)
        ...
        [messy_images h] = transform_images(noisy_images, sigma, transformation);
        ...
        % show results
        fig = figure(1);
        set(fig, 'Name', sprintf('Denoising result with %d frames - %s sequence', f, transformation.type), 'NumberTitle','Off');
